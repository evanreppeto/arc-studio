...
export const ARC_SKILLS: readonly ArcSkill[] = [
  ...
  },
  {
    ...
    instructions: [
      "Use this skill when the operator explicitly wants draft work products — most often a single creative asset (image, video, logo composite, or a short piece of copy) rather than a full campaign.",
      ...
      "Prefer the workspace's own approved media over generating something new. Generated creative should package, resize, extend, or test authentic proof — a real job, a real crew, a real result — not replace it. When approved media would serve the request, attach it and say so instead of generating.",
      "Match the format to the placement before generating: 1:1 and 4:5 for feed, 9:16 for stories and reels, 16:9 for display and video. Producing the wrong aspect ratio wastes the whole asset, so confirm the placement when the operator has not named one.",
      "Keep words out of the pixels. Text baked into a generated image renders unreliably and breaks at thumbnail size — put headlines, offers, and calls to action in the caption or an overlay layer the operator can edit, and say that is what you did.",
      "Never generate a logo or wordmark. A brand mark is an approved asset that gets composited in; an invented one is a defect, not a draft.",
      "Write the risk flags the reviewer will look for, honestly: embedded text or logo problems, an unrealistic or physically impossible scene, anything needing privacy redaction (faces, license plates, house numbers, visible addresses), and claim risk — before/after framing and damage imagery imply an outcome the business may not be able to promise.",
      "Judge the draft at the size it will be seen. A composition that only reads at full resolution has failed if it runs as a feed thumbnail.",
    ],
    outputContract: [
      "Every created work product must be pending human approval.",
      "Explain the source context and assumptions behind the draft.",
      "State the asset's provenance plainly: whether it is approved workspace media, newly generated, or a composite of both — and name the source media it builds on.",
      "Name the format and aspect ratio produced, and the placement it is intended for.",
      "List the risk flags that apply, or say explicitly that none do. Silence on risk reads as a clean review and is worse than a flagged draft.",
