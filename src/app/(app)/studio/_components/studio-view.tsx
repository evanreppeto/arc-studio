...
export function StudioView({ brandName, libraryItems, live = false, campaigns = [], mediaEnabled = false, brandPalette = [], brandTokens = null }: { brandName: string; libraryItems?: Item[]; live?: boolean; campaigns?: CampaignRef[]; mediaEnabled?: boolean; brandPalette?: string[]; brandTokens?: CanvasBrand | null }) {
  ...
  /** A revision that saved but never started. Not an error — nothing failed or
   *  was lost — so it gets warn tone, not the red genErr treatment. */
  ...
  const reviseDraft = async (d: StudioDraft) => {
    ...
    // Recorded, but the agent never acknowledged the wake, so it will not run
    // and nothing retries it. Silence here looked identical to success (BSR-695).
    setDraftNotice(res.dispatched === false ? "Revision saved, but Arc hasn't started on it — it won't begin on its own. Try again shortly." : null);
  };
            ...
            <div className="ipane">
              <div className="dwrap">
                ...
                <div className="psec">
                  ...
                  {draftNotice ? (
                    <div role="status" style={{ margin: "0 2px 8px", fontSize: 11, color: "var(--warn-text)", lineHeight: 1.4 }}>{draftNotice}</div>
                  ) : null}
