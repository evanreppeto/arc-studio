...
// An authenticated session lands on the home screen and is told what is waiting;
// without one — as in CI, where there is no Supabase env — it redirects on to
// the sign-in screen. Either way root resolves cleanly into the app.
//
// The headline is generated by `needsYouPhrase` (src/domain/vocabulary.ts), so
// match THAT rather than a hand-typed copy of it. This assertion hardcoded
// "waiting on you", which was the phrasing before #1045 consolidated four
// different wordings of the same idea into the vocabulary helper. The copy
// change was deliberate and correct; this assertion was simply left behind, and
// because `e2e` is a required check it left main red and blocked every PR.
test("/ enters the real app", async ({ page }) => {
  ...
  // needsYouPhrase(1) → "1 thing needs you"; needsYouPhrase(n) → "N things need you".
  await expect(page.locator("body")).toContainText(/things? needs? you|sign in/i);
